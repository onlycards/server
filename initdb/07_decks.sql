...
/*
  ...
  `fk_decks_owner` запрещает колоду без владельца.
  При удалении владельца его колоды удаляются каскадно.
  `fk_decks_front` и fk_decks_back запрещают языки, которых нет в справочнике.
...
CREATE TABLE `decks` (
  ...
  `front_language` CHAR(2) CHARACTER SET ascii NOT NULL,
  `back_language` CHAR(2) CHARACTER SET ascii NOT NULL,
  ...
  CONSTRAINT `fk_decks_owner`
    ...
    REFERENCES `users` (`id`)
    ON DELETE CASCADE,
  CONSTRAINT `fk_decks_front`
    FOREIGN KEY (`front_language`)
    REFERENCES `languages` (`code`)
    ON DELETE RESTRICT,
  CONSTRAINT `fk_decks_back`
